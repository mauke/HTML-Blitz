...
method _consume_entity_maybe($chunk) {
    ...
    if ($$src_ref =~ /\G#/gc) {
        if ($$src_ref =~ /\G[xX]/gc) {
            $$src_ref =~ /\G([[:xdigit:]]+)/gc
                or $self->_fail("missing hex digits after '&#x'");
            $char = chr hex $1;
        } else {
            $$src_ref =~ /\G(\d+)/agc
    ...
    if ($$src_ref =~ /\G</gc) {
        if ($$src_ref =~ /\G!/gc) {
            ...
            if ($$src_ref =~ /\Gdoctype/aaigc) {
                $$src_ref =~ /\G[ \t\n\f]+/gc
                    or $self->_fail("missing whitespace after '<!DOCTYPE'");
                $$src_ref =~ /\Ghtml/aaigc
                    or $self->_fail("invalid non-html doctype");
                $$src_ref =~ /\G[ \t\n\f]*>/gc
                    or $self->_fail("missing '>' after '<!DOCTYPE html'");
                return {
                    type => TT_DOCTYPE,
                    pos  => $pos,
                };
